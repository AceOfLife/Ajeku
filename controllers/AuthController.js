...
const login = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ 
      ...
      include: [{
        model: Client,
        as: 'client',  // Make sure this association exists in your User model
        attributes: ['id']
      }]
    });
    ...
    // Add clientId to payload if user is a client
    ...
    // Add clientId to response if available
    if (user.role === 'client' && user.client) {
      response.user.clientId = user.client.id;
    }
...
// User signup (updated to create client record if needed)
const signup = async (req, res) => {
  const { name, email, password, role } = req.body;
