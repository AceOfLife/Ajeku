...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const uploadDir = '/tmp/uploads'; // Vercel provides a temporary writable directory called /tmp
        if (!fs.existsSync(uploadDir)) {
            fs.mkdirSync(uploadDir, { recursive: true }); // Ensure uploads folder exists
        }
...
const upload = multer({
    storage,
}).any(); // Allow multiple files with .any()
    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            if (req.files && req.files.length > 0) {
                const imageUrls = req.files.map(file => ({
                    property_id: newProperty.id,
                    image_url: '/uploads/' + file.filename, // Store the temporary path (accessible during the request lifecycle)
                }));
