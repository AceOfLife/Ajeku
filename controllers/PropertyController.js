...
exports.createProperty = async (req, res) => {
    const { upload, uploadImagesToCloudinary, uploadDocumentsToCloudinary } = require('./multerConfig');
    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            if (req.files && req.files['documents']) {
                ...
                const documentDetails = await uploadDocumentsToCloudinary(documents);
                documentRecords = documentDetails.map(doc => ({
                    property_id: newProperty.id,
            ...
            // Return the filtered property in the response including documents
            res.status(201).json({
                property: filteredProperty,
                images: imageUrls || [],
                documents: documentRecords.map(doc => ({ url: doc.document_url, name: doc.document_name })) || []
            });
        } catch (error) {
            console.error(error);
            res.status(400).json({ message: 'Error creating property', error });
