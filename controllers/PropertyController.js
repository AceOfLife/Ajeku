...
}).any(); // Accepts multiple files, any type of field name
...
exports.createProperty = async (req, res) => {
    ...
    upload(req, res, async (err) => {
        ...
        // Continue processing after successful file upload
        try {
            const {
                name,
                size,
            ...
            const admin = req.user; // Assuming req.user is populated with logged-in admin's data
            if (admin.role !== 'admin') {
                return res.status(403).json({ message: 'You are not authorized to create a property' });
            }

            // Check if the agent exists (agent_id should refer to a user with role 'agent')
            const agent = await User.findByPk(agent_id, { where: { role: 'agent' } });
            if (!agent) {
                return res.status(404).json({ message: 'Agent not found' });
            }
            ...
            res.status(201).json(newProperty); // Send the newly created property in the response
        } catch (error) {
...
exports.deleteProperty = async (req, res) => {
    try {
        ...
        if (deleted) {
            return res.status(204).send(); // No content to send back after successful deletion
        }
