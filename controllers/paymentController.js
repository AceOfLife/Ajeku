...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (payment_type === "rental" && property.isRental) {
      ...
      let rental = await RentalPayment.findOne({
        where: { user_id, property_id }
      });
      ...
      if (rental) {
        // Extend existing rental (no room change)
        rental.expiry_date = oneYearLater;
        await rental.save();
        updatedProperty = property;
      } else {
        // New rental: decrement rooms
        await Property.decrement('number_of_rooms', {
          where: { id: property_id },
          by: 1
        });
        
        updatedProperty = await Property.findByPk(property_id);
        
        rental = await RentalPayment.create({
          user_id,
          property_id,
          start_date: today,
      ...
      return res.status(200).json({
        ...
        rentalDetails: {
          ...
          remainingRooms: updatedProperty.number_of_rooms
        }
      });
    }
